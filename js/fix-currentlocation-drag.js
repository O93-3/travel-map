// Fix: Current Location bubble drag should not deform (stable)
(function(){
  'use strict';
  function ready(fn){
  ...
  ready(function(){
    ...
    function forceLeftTop(x,y){
      // clear right/bottom first to avoid stretch
    ...
    // normalize on load
    (function(){
      try{ var pos=JSON.parse(localStorage.getItem(key)||'null');
        if(pos && typeof pos.x==='number' && typeof pos.y==='number'){ forceLeftTop(pos.x,pos.y); return; }
      }catch(_){ }
      var r=el.getBoundingClientRect();
      forceLeftTop(r.left,r.top);
    })();

    function isLocked(){ try{ return localStorage.getItem('locLocked')==='1'; }catch(_){ return false; } }
